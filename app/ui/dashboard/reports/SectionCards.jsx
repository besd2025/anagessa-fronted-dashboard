...
export function SectionCards() {
    ...
    const fetchReport = async () => {
      try {
        const response = await fetchData(
          "get",
          `mais/rapportages_sdl_ct/get_details/`,
        );
        setData({
          ...
          gap_total:
            response?.total_grains_rapport > response?.total_grains_mobile
              ? response?.total_grains_rapport - response?.total_grains_mobile
              : response?.total_grains_mobile - response?.total_grains_rapport,
          total_gap_ca:
            response?.total_grains_a_rapport > response?.total_grains_a_mobile
              ? response?.total_grains_a_rapport -
                response?.total_grains_a_mobile
              : response?.total_grains_a_mobile -
                response?.total_grains_a_rapport,
          total_gap_cb:
            response?.total_grains_b_rapport > response?.total_grains_b_mobile
              ? response?.total_grains_b_rapport -
                response?.total_grains_b_mobile
              : response?.total_grains_b_mobile -
                response?.total_grains_b_rapport,
          percentage_a:
            (response?.total_grains_a_rapport /
              response?.total_grains_rapport) *
            100,
          percentage_b:
            (response?.total_grains_b_rapport /
              response?.total_grains_rapport) *
            100,
          pourcentage_total_gap:
            response?.total_grains_rapport > response?.total_grains_mobile
              ? ((response?.total_grains_rapport -
                  response?.total_grains_mobile) *
                  100) /
                response?.total_grains_rapport
              : ((response?.total_grains_mobile -
                  response?.total_grains_rapport) *
          ...
          message:
            response?.total_grains_rapport > response?.total_grains_mobile
              ? "Le rapport a depassé la quantité collectée"
              : "Le rapport est inferieur à la quantité collectée",
        });
      } catch (error) {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 px-4 *:data-[slot=card]:bg-gradient-to-t *:data-[slot=card]:from-primary/5 *:data-[slot=card]:to-card *:data-[slot=card]:shadow-xs lg:px-6 @xl/main:grid-cols-2 @5xl/main:grid-cols-4 dark:*:data-[slot=card]:bg-card">
      ...
      <Card className="@container/card col-span-4 @5xl/main:col-span-2 relative">
        <CardHeader>
          <CardDescription className="text-lg  font-semibold tabular-nums  ">
            GAP
          </CardDescription>
            ...
            <div className="flex flex-row gap-x-2 items-center bg-primary/10 py-1 px-2 rounded-lg w-max">
              ...
              <CardDescription className="font-semibold text-accent-foreground text-lg">
                {data?.total_gap_ca >= 1000 ? (
                  <>
                    {(data?.total_gap_ca / 1000).toLocaleString("fr-FR", {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}{" "}
                    <span className="text-sm">T</span>
            ...
            <div className="flex flex-row gap-x-2 items-center bg-secondary/10 py-1 px-2 rounded-lg">
              ...
              <CardDescription className="font-semibold text-accent-foreground text-lg">
                {data?.total_gap_cb >= 1000 ? (
                  <>
                    {(data?.total_gap_cb / 1000).toLocaleString("fr-FR", {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}
                    <span className="text-sm">T</span>
        ...
        <CardFooter className="flex-col items-start gap-1.5 text-sm">
          <div className="line-clamp-1 flex gap-2 font-medium">
            Gap de {data?.pourcentage_total_gap?.toFixed(2)}%{" "}
